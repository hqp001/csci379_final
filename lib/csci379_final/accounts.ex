defmodule Csci379Final.Accounts do
  @moduledoc """
  ...
  alias Csci379Final.Accounts.{User, UserToken, UserNotifier, OauthIdentity}
  ...
  See `Csci379Final.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Csci379Final.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  @doc """
  Delivers a welcome email to a newly registered user.
  """
  ...
  def find_or_create_user_from_oauth(%{provider: provider, uid: uid, email: email}) do
        ...
        Repo.transact(fn ->
          user =
            case Repo.get_by(User, email: email) do
              nil -> create_oauth_user(email)
              existing -> existing
            end

          with {:ok, user} <- ensure_user_ok(user),
               {:ok, _identity} <-
                 Repo.insert(%OauthIdentity{provider: provider, uid: uid, user_id: user.id}) do
            {:ok, user}
          end
        end)
    end
  ...
  defp ensure_user_ok({:ok, user}), do: {:ok, user}
  defp ensure_user_ok(%User{} = user), do: {:ok, user}
  defp ensure_user_ok(err), do: err

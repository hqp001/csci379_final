  ...
  def render(assigns) do
    ...
    <div class="max-w-3xl mx-auto p-6">
      ...
      <div class="mt-8 space-y-3">
        <div :for={chapter <- @story.chapters} class="rounded-xl border border-slate-200 bg-white">
          <button
            phx-click="toggle_chapter"
            phx-value-id={chapter.id}
            class="flex w-full items-center justify-between px-5 py-4 text-left hover:bg-slate-50 rounded-xl transition-colors"
          >
            <div>
            ...
            <svg
              ...
              class={["h-5 w-5 text-slate-400 transition-transform", MapSet.member?(@open_chapters, chapter.id) && "rotate-180"]}
              viewBox="0 0 20 20"
              fill="currentColor"
            >
          ...
          <div :if={MapSet.member?(@open_chapters, chapter.id)} class="border-t border-slate-200 px-5 py-3">
            <p :if={chapter.description} class="mb-3 text-sm text-slate-500">{chapter.description}</p>
            <div class="space-y-2">
              <div :for={scene <- chapter.scenes} class={[
                "flex items-center justify-between rounded-lg px-4 py-3",
                scene.is_locked && "bg-slate-50",
                !scene.is_locked && "bg-indigo-50"
              ]}>
                <div class="flex items-center gap-3">
                  <div :if={scene.is_locked} class="text-slate-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                      <path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd" />
                    </svg>
                  </div>
                  <div :if={!scene.is_locked} class="text-indigo-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                  ...
                  <div>
                    <p class={["text-sm font-medium", scene.is_locked && "text-slate-400"]}>
                      {scene.title}
                    </p>
                    <p class="text-xs text-slate-400">{length(scene.quests)} quests</p>
                  </div>
                </div>
                <.link
                  :if={!scene.is_locked}
